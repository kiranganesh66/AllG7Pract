...
export class Game extends React.Component {
  ...
  get fetchGameState() {
    ...
    } else if (this.game.isStalemate) state.message = `Stalemate`;
    else state.message = `Next Player '${this.game.currentPlayer}'`;

    return state;
  }

  handleMove = (id) => {
    if (this.game.move(id) === false) return;
      ...
      <div className="body">
        <div className="game-component">
          ...
          <div className="same-row">
            <GameBoard cells={this.state.cells} onCellClick={this.handleMove} />
            <GameMoves moves={this.state.moves} />
          </div>
        </div>
      </div>
    );
