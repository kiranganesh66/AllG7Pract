...
class BookManager {
  ...
  //// 13-7-2024
   ...
   multiMatch(match){
    ...
    function closurematcher(book){
        return book.match.toLowerCase().includes(match.toLowerCase())
    }
 
     function _closurematcher(book){
        book.match.includes(match)
     }
    
     if(typeof match === String){
      return  closurematcher(this.book)
     }else{
        return  _closurematcher(this.book)
     }
...
filtermulitipleMatch(authoer){

    this._books.filter(this.multiMatch(authoer));
}
