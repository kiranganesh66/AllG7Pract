...
class BookManager {
  ...
  // sortTheitems(returnValue, order = "asc") {
  //   if (!["asc", "desc"].includes(order)) {
  //     throw new Error(
  //       "Incorrect order, accepted order params are asc and desc"
  //     );
  //   }
  ...
  //     for (var i = 0; i < n - 1; i++) {
  //       let ascCondition =
  //         (returnValue ? returnValue(current.value) : current.value) >
  //         (returnValue ? returnValue(current.next.value) : current.next.value);
  //       let descCondition =
  //         (returnValue ? returnValue(current.value) : current.value) <
  //         (returnValue ? returnValue(current.next.value) : current.next.value);
